...
const CustomizedPieLabel = props => {
   ...
   if (lastRenderedPieTextItemPositionList) {
      const mostClosestPoint = lastRenderedPieTextItemPositionList.filter(position => {
         return uint(position.textItemYPosition - textItemYPosition) < props.fontSize;
      });
      if (mostClosestPoint.length > 0) {
         ...
         if (uint(mostClosestObject.textItemYPosition - textItemYPosition) < props.fontSize) {
            if (mostClosestObject.textItemYPosition - textItemYPosition > 0) {
               pieLabelItem.ey += props.fontSize + uint(mostClosestObject.textItemYPosition - textItemYPosition);
            } else {
   ...
   props.setLastRenderedPieTextItemPosition(textItemXPosition, textItemYPosition, pieLabelItem.textAnchor);
   return (
      <g>
