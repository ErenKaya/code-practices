...
const CustomizedPieLabel = props => {
   ...
   let pieLabelItem = PieLabelItem.preparePieLabelPositionObject(midAngle, outerRadius, cx, cy, 30);
   ...
   if (lastRenderedPieTextItemPosition) {
      logObject = {
         ...logObject,
         diffrence: lastRenderedPieTextItemPosition.textItemYPosition - textItemYPosition,
         lastRenderedTextYPos: lastRenderedPieTextItemPosition.textItemYPosition
      };
      if (uint(lastRenderedPieTextItemPosition.textItemYPosition - textItemYPosition) < props.fontSize) {
         if (lastRenderedPieTextItemPosition.textItemYPosition - textItemYPosition > 0) {
            pieLabelItem.ey += props.fontSize + uint(lastRenderedPieTextItemPosition.textItemYPosition - textItemYPosition);
         } else {
         ...
         textItemXPosition = pieLabelItem.ex + (pieLabelItem.cos >= 0 ? 1 : -1) * 2;
         textItemYPosition = pieLabelItem.ey;
         logObject["changesAndCurrent"] = textItemYPosition;
      }
      console.table(logObject);
   }
   props.setLastRenderedPieTextItemPosition(textItemXPosition, textItemYPosition);
   return (
      <g>
         <text x={pieLabelItem.cx} y={pieLabelItem.cy} dy={8} textAnchor="middle" fill={fill}>
            {payload.name}
         </text>
         <Sector
            cx={pieLabelItem.cx}
         ...
         />
         <path
            d={`M${pieLabelItem.sx},${pieLabelItem.sy}L${pieLabelItem.mx},${pieLabelItem.my}L${pieLabelItem.ex},${pieLabelItem.ey}`}
            stroke={"red"}
            fill="none"
         />
         <circle cx={pieLabelItem.ex} cy={pieLabelItem.ey} r={2} fill={fill} stroke="none" />
         <text
            x={textItemXPosition}
            y={textItemYPosition}
            fontSize={9}
            ...
            <PieChart width={800} height={400}>
               <Pie
                  ...
                  label={
                     <CustomizedPieLabel
                        fontSize={9}
                        setLastRenderedPieTextItemPosition={this.setLastRenderedPieTextItemPosition}
                        getLastRenderedPieTextItemPosition={this.getLastRenderedPieTextItemPosition}
                     />
                  }
               />
            </PieChart>
